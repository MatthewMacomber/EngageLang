...
from engage_errors import EngageError, ErrorAggregator, create_runtime_error, create_type_error, create_name_error
...
def setup_global_environment():
    """Set up the global environment with all integrated features."""
    ...
    # Load built-in functions into the global symbol table
    stdlib.load_builtin_functions(global_symbol_table, BuiltInFunction)
    
    # Load UI component built-in functions
    if UI_AVAILABLE:
        try:
            for func_name, func_obj in UI_BUILTIN_FUNCTIONS.items():
                global_symbol_table.set(func_name, func_obj)
        except Exception as e:
    ...
    for module_name in modules_to_load:
        try:
            stdlib.load_module_functions(module_name, global_symbol_table, BuiltInFunction)
        except Exception as e:
